...
LeetTrace is a local LeetCode practice workspace for tracking problem-solving progress, running submissions, managing curated lists, and scheduling reviews with spaced repetition.
...
Built with type safety, scalability, and developer experience in mind for local coding-interview practice.
...
- [Docker](https://www.docker.com) - Containerization tool for deploying LeetTrace
...
```
git clone <your-repo-url> leettrace
```
...
The TanStack Query and Router devtools are wired up and active in `src/App.tsx`. The TanStack Table and React Hook Form devtools are demonstrated in the example feature (`src/features/example`) — see the references below.
...
A working version of the above lives in `src/features/example/MatchesTable.tsx`.

**React Hook Form DevTools** icon can be recognized in the top right corner of the page by the pink React Hook Form clipboard logo. A utility component has also provided. Like the TanStack Table Devtools component above, a prop must be passed from a specific hook. In this case, it is the control prop from the `useForm()` hook. Similar to TanStack Table, use of React Hook Form DevTools requires the component be added to each unique form. A working version lives in `src/features/example/MatchForm.tsx`. More information can be found in the [React Hook Form DevTools documentation](https://react-hook-form.com/dev-tools).
...
**Zustand** provides a built-in [devtools middleware](https://github.com/pmndrs/zustand#redux-devtools) for use with [Redux DevTools](https://github.com/reduxjs/redux-devtools#redux-devtools). `src/store/useExampleStore.ts` shows the middleware wired up.
...
## Deploying to Dev (Docker Compose)

The full stack — MongoDB, the backend API (with code formatters baked in), and
the nginx-served frontend — is containerized so a dev deploy is a single
command. You do **not** install the formatters one by one on the host.

```bash
docker compose up --build
```

- Frontend: http://localhost:8080 (nginx serves the built app and reverse-proxies
  `/api` to the backend)
...
| --- | --- | --- |
| `mongo` | `mongo:8` | Root credentials from `MONGO_USER` / `MONGO_PASSWORD` (default `leetcode`/`leetcode`). |
| `api` | `server.Dockerfile` | Node + `tsx`; bakes in `clang-format`, a JRE + `google-java-format` jar, and `black`. Prettier ships via pnpm. |
| `web` | `Dockerfile` | Multi-stage: builds the Vite app, serves it with nginx + `/api` proxy. |

### Configuration

Compose reads a root `.env` file for `${VAR}` interpolation; every value has a
safe default, so the stack boots without one. To enable LeetCode run/submit and
the AI provider, set these in `.env`:

```dotenv
LEETCODE_SESSION=...
CSRF_TOKEN=...
OPENAI_API_KEY=...
# Optional: override the bundled Mongo credentials
MONGO_USER=leetcode
MONGO_PASSWORD=leetcode
```

The backend's `MONGODB_URI` and `GOOGLE_JAVA_FORMAT_JAR` are set automatically by
`docker-compose.yml` — no manual wiring needed.

### Code formatters

All four formatters used by `POST /api/format` are provided by the `api` image,
so every language formats out of the box in the container:

| Language | Formatter | Source |
| --- | --- | --- |
| JS / TS | Prettier | pnpm dependency |
| Python | `black` | apt |
| C / C++ | `clang-format` | apt |
| Java | `google-java-format` | jar baked into the image |

Missing a formatter only affects that one language (the endpoint returns a clear
"not installed" message); the rest keep working.
